...
function(i) {
    ...
    this.configTimePicker = function(e) {
        ...
        return r.today = n(new Date).getTime(), r.time = {}, r.increaseHours = function() {
            r.time.hours = Number(r.time.hours), 12 == r.mainObj.format && (12 != r.time.hours ? r.time.hours += 1 : r.time.hours = 1), 24 == r.mainObj.format && (Math.min(23, r.mainObj.maxHour - 1) != r.time.hours ? r.time.hours = (r.time.hours + 1) % 24 : Math.max(0, r.mainObj.minHour)), r.time.hours = r.time.hours < 10 ? "0" + r.time.hours : r.time.hours
        }, r.decreaseHours = function() {
            r.time.hours = Number(r.time.hours), 12 == r.mainObj.format && (r.time.hours > 1 ? r.time.hours -= 1 : r.time.hours = 12), 24 == r.mainObj.format && (r.time.hours > r.mainObj.minHour ? r.time.hours = (r.time.hours + 23) % 24 : r.time.hours = r.mainObj.minHour), r.time.hours = r.time.hours < 10 ? "0" + r.time.hours : r.time.hours

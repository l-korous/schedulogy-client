!function (d, str) {
    ...
    this.$get = ["$rootScope", "$ionicPopup", function ($rootScope, dialog) {
            ...
            function format(offset, formatString) {
                ...
                /** @type {string} */
                settings.time.hours = Math.floor(settings.time.hours);
                /** @type {string} */
                settings.time.minutes = settings.time.minutes.toFixed(0);
                settings.time.hoursForShow = settings.time.hours ? (settings.time.hours < 10 ? "0" + settings.time.hours : settings.time.hours) : '00';
            ...
            return settings.today = clearTime(new Date).getTime(),
                settings.time = {},
                settings.timeInMinutes = function () {
                    var whatToCompare;
                    if (24 === settings.mainObj.format)
                        whatToCompare = parseInt(settings.time.hours);
                    else {
                        if (settings.time.meridian === 'AM')
                            whatToCompare = parseInt(settings.time.hours % 12);
                        else
                            whatToCompare = parseInt((settings.time.hours % 12) + 12);
                    }
                    return whatToCompare * 60 + parseInt(settings.time.minutes);
                },
                settings.increaseHours = function () {
                    if (settings.mainObj.constraint) {
                        var compareTo = settings.mainObj.constraint.to;
                        var whatToCompare = settings.timeInMinutes() + 60;
                    ...
                    if (12 == settings.mainObj.format) {
                        ...
                        else if (settings.time.meridian === 'AM')
                            settings.time.hours = 1;
                        else
                            return false;
                    }
                    if (24 == settings.mainObj.format) {
                        /** @type {number} */
                        settings.time.hours = settings.time.hours == 23 ? settings.time.hours : (settings.time.hours + 1);
                    }

                    settings.time.hoursForShow = settings.time.hours ? (settings.time.hours < 10 ? "0" + settings.time.hours : settings.time.hours) : '00';
                    return true;
                },
                settings.processHoursChange = function () {
                    var resetToPrevious = false;
                    if (isNaN(settings.time.hours))
                        resetToPrevious = true;
                    else {
                        if (settings.mainObj.constraint) {
                            var compareTo = settings.mainObj.constraint.to;
                            var whatToCompare = settings.timeInMinutes();
                            if (whatToCompare > compareTo)
                                resetToPrevious = true;

                            compareTo = settings.mainObj.constraint.from;
                            whatToCompare = settings.timeInMinutes();
                            if (whatToCompare < compareTo)
                                resetToPrevious = true;
                        }
                    }
                    if (resetToPrevious)
                        settings.time.hours = settings.storedPreviousHours;
                    else
                        settings.storedPreviousHours = settings.time.hours ? settings.time.hours : 0;
                ...
                settings.decreaseHours = function () {
                    ...
                    settings.time.hours = Number(settings.time.hours);
                    if (12 == settings.mainObj.format) {
                        if (settings.time.hours == 12 && settings.time.meridian === 'AM') {
                            // do nothing.
                            return false;
                        }
                        else if (settings.time.hours > 1) {
                            settings.time.hours -= 1;
                        } else
                            settings.time.hours = 12;
                    }
                    if (24 == settings.mainObj.format) {
                        /** @type {number} */
                        settings.time.hours = settings.time.hours == 0 ? settings.time.hours : ((settings.time.hours + 23) % 24);
                    }
                    /** @type {(number|string)} */
                    settings.time.hoursForShow = settings.time.hours ? (settings.time.hours < 10 ? "0" + settings.time.hours : settings.time.hours) : '00';
                    return true;
                ...
                settings.increaseMinutes = function () {
                    if (settings.mainObj.constraint) {
                        var compareTo = settings.mainObj.constraint.to;
                        var whatToCompare = settings.timeInMinutes() + settings.mainObj.step;
                        if (whatToCompare > compareTo)
                            return false;
                    }
                    ...
                    /** @type {number} */
                    var old_minutes = settings.time.minutes;
                    settings.time.minutes = (settings.time.minutes + settings.mainObj.step) % 60;
                    if (settings.time.minutes == 0) {
                        if (!settings.increaseHours())
                            settings.time.minutes = old_minutes;
                    }
                    /** @type {(number|string)} */
                ...
                settings.time.hoursForShow = settings.time.hours ? (settings.time.hours < 10 ? "0" + settings.time.hours : settings.time.hours) : '00';
                settings.storedPreviousHours = settings.time.hours;
                buttons.push({
                ...
                buttons.push({
                    ...
                    type: "button_close",
                    onTap: function(t) {
                        settings.mainObj.closeCallback();
                    }
                });
